...
function generateProperMasterFingerprint() {
  ...
  // The fingerprint is a Buffer, so we need to convert it to hex properly
  const fingerprintBuffer = masterKey.fingerprint;
  const masterFingerprint = fingerprintBuffer.toString("hex").toUpperCase();

  console.log(
    `Generated master fingerprint: ${masterFingerprint} (${masterFingerprint.length} chars)`
  );
  console.log(`Fingerprint buffer: [${Array.from(fingerprintBuffer)}]`);
