...
export class CryptoHDKey extends RegistryItem implements ICryptoKey {
  ...
  public getBip32Key = () => {
    ...
    if (this.isMaster()) {
      ...
      depth =
        this.getOrigin()?.getComponents().length ||
        (this.getOrigin()?.getDepth() as number);
      const paths = this.getOrigin()?.getComponents() as PathComponent[];
      const lastPath = paths[paths.length - 1];
      if (lastPath) {
        index = lastPath.isHardened()
          ? lastPath.getIndex()! + 0x80000000
          : lastPath.getIndex()!;
  ...
  public getOutputDescriptorContent = () => {
    ...
    if (this.getOrigin()) {
      if (
        this.getOrigin()?.getSourceFingerprint() &&
        this.getOrigin()?.getPath()
      ) {
